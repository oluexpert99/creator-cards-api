...
function Server(serverConfig = {}) {
    ...
    app[method](path, async (expressRequest, expressResponse) => {
      ...
      try {
        ...
        responseComponents.body.message = error.isApplicationError
          ? error.message
          : 'Some error occured.';
        // Surface the business-rule error code on the wire as `code` (contract requirement).
        // Skip the framework validator's SPCL_VALIDATION so its responses keep their native shape.
        if (error.errorCode && error.errorCode !== 'SPCL_VALIDATION') {
  ...
  return {
    ...
    addHandler,
    executeRequest,
    // Expose the raw express app so application code can mount middleware that needs the app
    // directly (e.g. Swagger UI static assets), which the JSON-only handler path can't serve.
    app,
  };
}
